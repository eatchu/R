...
# depth 이산형 변수 리코딩
# 1. depth변수 범위
range(quakes$depth)# depth 범위 : 연속형변수
# 40 680
...
qplot(hwy, data=mpg, fill=drv, binwidth=4) # binwidth 옵션 적용 
...
qplot(wt, mph, data=mtcars, geom = c('point', 'line'))
...
p + geom_count() + geom_smooth(method = 'lm') #회귀모형 추가


cor(galton$parent,galton$child) #0.4587624 어느 정도의 상관관계가 있다
#  +-0.3~0.4 이상이면 두개의 칼럼사이의 상관관계 존재
...
# [단계2] 지도정보 생성
daegu <- get_stamenmap(daegu,zoom=7,maptype='watercolor')
