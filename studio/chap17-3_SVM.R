...
pred
# 1 2 3 4 5 6 7 <- index
# 1 1 1 1 0 0 0 <- predict
# Levels: 0 1


# 3. kernel="linear" 변경 : 선형분류
# model tuning -> gamma, cost, kernel
# cost = 1 : 오분류 조절 속성(분류정확도 높임) - 오분류 감소, 과적합 증가
# kernel = "radial"
# gamma = if (is.vector(x)) 1 else 1 / ncol(x) : 결정경계 모양 조절
# x변수=2개 -> 1/2 -> gamma=0.5 - 값이 큰 경우 경계 공간 작아짐, 오분류 감소, 과적합 증가
...
# 분류정확도
42/45 #0.9333333 -> 비선형분류
43/45 #0.9555556 -> 선형분류
