#chap12_CrossTableChi-squared(1)
...
# Chi^2 =  2.766951 : 각 셀 기대비율의 합 
# d.f. =  2 : 검정통계량    
# p =  0.2507057 : 유의확률 - 귀무가설 채택 

# 귀무가설 : 부모의 학력수준과 자녀의 대학진학 여부는 관련성이 없다.
# 연구가설 : 부모의 학력수준과 자녀의 대학진학 여부는 관련성이 있다.

# [부모와 자식의 학력은 상관관계가 없다]
# α > P값 : 연구가설 채택(귀무가설 기각) 
# α ≦ P값 : 연구가설 기각(귀무가설 채택)



# 카이 값
chi_2<-0.544+0.363+0.684+1.023+0.091+0.060   
# d.f 자유도 : 샘플수(n)에서 자유롭게 선택할 수 있는 수 (행-1)*(열-1)
d.f <- (3-1)*(2-1)
...
# 기대도수 : 10(1), 10(2), 10(3), 10(4), 10(5), 10(6)
...
# 귀무가설 기각, 대립가설 채택 : 스포츠 음료의 종류에 따라 선호도의 차이가 있다.


#2) 이원카이제곱 - 교차분할표 이용 (변인이 두개)
...
# 독립변수(x)와 종속변수(y) 생성 
x <- df$level2 # 부모의 학력수준
y <- df$pass2 # 자녀의 대학진학여부 
...
data <- subset(data1, !is.na(survey), c(method, survey)) 
d <- data1[-1]
