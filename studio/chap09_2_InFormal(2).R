...
# 1.텍스트 파일 가져오기
#----------------------------------------------------
marketing <- file("C:/IITT/2_Rwork/Part-II/marketing.txt", encoding="UTF-8") 
marketing2 <- readLines(marketing) # 줄 단위 Test file 읽기

close(marketing) # 처음 가죠온 부분 지운다
head(marketing2) # 앞부분 6줄 보기 - 줄 단위 문장 확인 
str(marketing2) # 472문장


# 2. 줄 단위 단어 추출
#----------------------------------------------------
# Map()함수 이용 줄 단위 단어 추출 
lword <- Map(extractNoun, marketing2) #Map(명사추출fuc,doc) : 리스트형태로 단어 추출
head(lword)
length(lword) # [1] 472
class(lword) # list
lword[1]
