...
wordcloud(df$word, df$freq, min.freq=2, random.order=FALSE, scale=c(4,0.7),
...
# - 출현빈도수 -> 비율 가중치 조정 -> 역수로 설정해서 중요도 측정
...
str(sms_tdm2)
rsum2 <- sort(rowSums(sms_tdm2), decreasing=TRUE) 
rsum2[1:10]
sms_tdm1[1,]
...
setwd("C:/IITT/2_Rwork/Part-IV")
sms_data <- read.csv('sms_spam.csv', stringsAsFactors = FALSE)
str(sms_data) #5558 obs. of  2 variables
...
# 6. csv file 
write.csv(sms_dtm_df,'C:/IITT/2_Rwork/Part-IV/sms_dtm_df.csv',
