...
# p-value = 0.0003422 : 귀무가설 기각 
# [해설] 두 교육방법에 따라 만족도의 차이가 있다
#(2) chisq.test
chisq.test(data$method,data$survey) # p-value = 0.0003422


# # 방향성이 있는 대립가설 검정  
prop.test(c(110,135), c(150, 150), alternative="greater", conf.level=0.95) #p-value = 0.9998
# 1교육방법의 만족도가 2교육방법의 만족도 보다 크다 (x)
prop.test(c(110,135), c(150, 150), alternative="less", conf.level=0.95) #p-value = 0.0001711
# 1교육방법의 만족도가 2교육방법의 만족도 보다 작다 (o)
...
mean(method1_score, na.rm=TRUE) # 5.556881
mean(method2_score, na.rm=TRUE) # 5.80339

# 4. 분포모양 검정 : 두 집단의 분포모양 일치 여부 검정
var.test(method1_score, method2_score) #p-value = 0.3002 : 동질성 분포 -> 모수검정  
# 동질성 분포 : t.test()
# 비동질성 분포 : wilcox.test()

# 5. 가설검정 - 두집단 평균 차이검정
t.test(method1_score, method2_score) #p-value = 0.0411 : 귀무가설 기각
...
# 3. 분포모양 검정 
var.test(before, after, paired=TRUE) #p-value = 0.4738
# 동질성 분포 : t.test()
# 비동질성 분포 : wilcox.test()
