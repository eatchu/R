...
# 4) tm 설치 
install.packages("tm") #텍스트 마이닝 : 텍스트 전처리, DTM, TDM
...
str(facebook_data) # chr [1:76] 백터 타입
facebook_data[76]


## 3. 세종 사전에 신규 단어 추가 
# term : 추가단어 , ncn : 명사 지시 코드
...
extractNoun("나는 김진성이고, 한국 사람이다.") #명사들만 출력됨
# "나" "김진성" "한국" "사람"  
paste(extractNoun("나는 김진성이고, 한국 사람이다."),collapse='')
# "나김진성한국사람"
...
facebook_nouns #원문과 함수에서 반환된 값이 같이 나옴

# (3) 단어 추출 결과
str(facebook_nouns) 
facebook_nouns[1] 
facebook_nouns[2]
length(facebook_nouns[1]) #1
...
# TermDocumentMatrix : [term(행),document(열)] : list
myCorpusPrepro_term
str(myCorpusPrepro_term)
# <<TermDocumentMatrix (terms: 696, documents: 76)>>
# Non-/sparse entries: 1256/51640
# Sparsity           : 98% : 희소비율(1의비율)
# Maximal term length: 12 :6음절
# Weighting          : term frequency (tf)
inspect(myCorpusPrepro_term[,1])

# (2) Corpus -> 평서문 변환 : matrix -> data.frame 변경
myTerm_df <- as.data.frame(as.matrix(myCorpusPrepro_term)) 
myTerm_df[1]
str(myTerm_df)
myTerm_df[c(1:10),1] #1 1 1 1 2 1 1 1 3 1:1열에서 처음 10개의 단어 빈도수

## 7. 단어 빈도수 구하기
# (1) 단어 빈도수 내림차순 정렬
# rowSums : 행단위 합게구하기 
wordResult <- sort(rowSums(myTerm_df), decreasing=TRUE) #백터데이터로 반환됨
#백터에는 출현빈도수 숫자가 입력되고 그 백터의 이름이 지정됨
wordResult[1:10] #출현 빈도수가 높은 단어 top10
names(wordResult[1:5]) # "데이터" "분석"  "빅데이터" "처리"  "사용"   


# (2) 불용어 제거 : 단어 빈도수 확인 후 의미없는 단어 제거 
# [단계1] 데이터 전처리 : 말뭉치 대상 전처리
myCorpusPrepro <-tm_map(myCorpusPrepro, removeWords, 
                        c(stopwords('english'),'사용','얘기')) # 불용어제거
# [단계2] 단어 문서 행렬 생성
# 단어 선별 -> matrix 변경
myCorpusPrepro_term <- TermDocumentMatrix(myCorpusPrepro, 
                                          control=list(wordLengths=c(4,16))) #2-8글자이내인것
...
# (1) 단어 이름 생성 -> 빈도수의 이름
myName <- names(wordResult)  #단어 이름들 추출

# (2) 단어이름과 빈도수로 data.frame 생성 : 행에 번호 부여하기
word.df <- data.frame(word=myName, freq=wordResult) #wordResult값을 df로 만듬
head(word.df)
