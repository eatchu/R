...
sentimental = function(sentences, posDic, negDic){
  ...
  scores = laply(sentences, function(sentence, posDic, negDic) {
    ...
    # 문장의 전처리 gsub(패턴,교체,문장) : 문장에 있는 특정패턴을 교체하는 함수
    sentence = gsub('[[:punct:]]', '', sentence) #문장부호 제거 '[[:punct:]]'
    sentence = gsub('[[:cntrl:]]', '', sentence) #특수문자 제거 '[[:cntrl:]]'
    sentence = gsub('\\d+', '', sentence) # 숫자 제거 '\\d+'
    ...
    # 점수 = 긍정단어 점수 - 부정단어 점수 : TRUE가 1의 값을 가짐 
